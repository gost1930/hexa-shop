...
const OrderForm: React.FC<OrderFormProps> = ({ increment, decrement, quantity, total, pro, handleQuantity = true }) => {
    const { isOpen, setIsOpen, onClose } = useDisCloser();
    const [isHouse, setIsHouse] = useState<boolean>(true);
    const { register, handleSubmit, formState: { errors } , reset} = useForm();
    const dispatch = useDispatch();

    const proIds = typeof pro?.id === "object" ? pro.map((item: any) => item.id) : [pro?.id];
        ...
        <>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="grid grid-cols-2">
                    {
                        handleQuantity && (<div className="flex items-center">
                            <button
                                onClick={increment}
                                type="button"
