...
const CategoryPro = () => {
    ...
    console.log(proName)
    const url = `product/getByCategoryName/${path}?page=${currentPage}&limit=${pageSize}${proName ? `&name=${proName}` : ""}`;
    ...
    // if (!data?.products) return <div className="h-screen"> <NoDataPage /></div>
    ...
    return (
        <main className="container md:mx-auto w-full md:min-h-screen">
            ...
            <section className="px-5 md:px-20 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                ...
                {loading ? <Spiner /> : error || pro?.length === 0 ? <NoDataPage /> :
                    pro?.map((p, index) => (
